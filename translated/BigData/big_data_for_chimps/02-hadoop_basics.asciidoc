...
你将会看到一个作业列表, the only entry being the job you just executed.  map 和 reduce 列分别表示 mappers 和 reducers 的完成百分比. 点击作业的ID会跳转到该作业的汇总页面. 该页面的左边是作业的汇总信息: 用户, 状态, 指向日志的链接, map 和 reduce 的 数量以及作业执行时间. 你还可以查看作业执行日志, 这将有助于作业问题的调试.
